    ...
    RETURN QUERY
        with current as (select c.id                                          as customer_id,
                         ...
                         from report_item ri
                                  ...
                                  left join warehouse_receipt wr on ri.warehouse_receipt_id = wr.id
               ...
               ROUND(
                       CASE
                           WHEN sum(cu.total_quantity) > 0
                               THEN
                               COALESCE(SUM(cu.total_amount),
                                        + SUM(cum.total_amount) + coalesce(SUM(a.adjustment_amount), 0), 0)
                                   /
...
alter function get_monthly_report_by_year_and_month(integer, integer, integer) owner to postgres;
