...
public interface ReportRepository extends JpaRepository<Report, Long>, JpaSpecificationExecutor<Report> {
        @Query(value = """
                    ...
                    MonthlySales AS (
                        ...
                        WHERE r.jalali_year = :jalaliYear AND r.month <= :month AND p.product_type = :productType
                    ...
                    SELECT
                        ac.customerName,
                        CAST(COALESCE(cs.totalQuantity, 0) AS BIGINT) AS totalQuantity,
                        CAST(COALESCE(cs.totalAmount, 0) AS NUMERIC) AS totalAmount,
                        CAST(COALESCE(cs.cumulativeTotalQuantity, 0) as BIGINT) AS cumulativeTotalQuantity,
                        CAST(COALESCE(cs.cumulativeTotalAmount, 0) AS NUMERIC) AS cumulativeTotalAmount,
                        CAST(ROUND(COALESCE(cs.totalAmount, 0) / NULLIF(cs.totalQuantity, 0), 0) AS NUMERIC) AS avgUnitPrice,
                        CAST(ROUND(COALESCE(cs.totalAmount, 0) / (SELECT totalSales FROM TotalSales) * 100, 2) AS NUMERIC) AS relativePercentage
                ...
                """, nativeQuery = true)
        List<Object[]> getSalesByYearGroupByMonthFilterByProductType(@Param("jalaliYear") int jalaliYear, @Param("month") int month, @Param("productType") int productType);
    ...
    List<Object[]> getSalesByYearGroupByMonth(@Param("yearId") Integer yearId, @Param("productType") int productType);
